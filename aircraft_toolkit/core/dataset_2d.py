...
from ..models.military import B52Bomber, C130Transport, F15Fighter
...
class Dataset2D:
        ...
        for i in tqdm(range(num_samples), desc=f"Generating {split_name}"):
            ...
            if self.task_mode in ["pose", "both"]:
                annotation["aircraft_pose"] = {
                    ...
                    "rotation": {"pitch": pose["pitch"], "yaw": pose["yaw"], "roll": pose["roll"]},
                }
    ...
    def _save_coco_format(self, annotations: list[dict], output_dir: str, split_name: str):
        ...
        for ann in annotations:
            ...
            coco_annotation = {"id": annotation_id, "image_id": ann["scene_id"], "iscrowd": 0}
